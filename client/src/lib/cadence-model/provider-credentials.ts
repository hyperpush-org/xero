  ...
  .superRefine((payload, ctx) => {
    if (payload.providerId === 'openai_codex') {
      ctx.addIssue({
        ...
        path: ['providerId'],
        message:
          'Cadence persists OpenAI Codex credentials through the OAuth login flow, not the credential upsert command.',
      })
    }
    ...
    if (payload.kind === 'oauth_session') {
      ctx.addIssue({
        ...
        path: ['kind'],
        message:
          'Cadence persists OAuth credentials through the OAuth login flow, not the credential upsert command.',
      })
    }
  })
