...
function getCheckpointResumability(card: CheckpointControlLoopCardAccumulator): {
  ...
  return {
    ...
    resumabilityDetail:
      'Xero has not observed enough durable approval or resume evidence to determine whether this boundary is resumable.',
    isResumable: false,
  }
}
  ...
  if (artifactRecommendation) {
    return {
      ...
      recoveryRecommendationDetail:
        ...
        'Xero projected durable recovery guidance for this action from advanced failure evidence.',
    }
  }
...
function getCheckpointTruthSource(card: CheckpointControlLoopCardAccumulator): {
  ...
  return {
    ...
    truthSourceDetail:
      'Xero recovered this boundary from recent resume or evidence history after the live row disappeared.',
  }
}
...
function getCheckpointLiveState(card: CheckpointControlLoopCardAccumulator): {
  ...
  if (card.latestResume?.status === 'started') {
    return {
      ...
      liveStateDetail:
        'The live action-required row is no longer present for this boundary, so Xero is showing the latest durable resume outcome instead.',
...
function getCheckpointDurableState(card: CheckpointControlLoopCardAccumulator): {
  ...
  if (latestPolicyDenied) {
    return {
      ...
      durableStateDetail:
        ...
        'Xero recorded a durable shell-policy denial for this command.',
  ...
  if (card.latestResume?.status === 'started') {
    return {
      ...
      durableStateDetail:
        'Xero has already cleared the durable approval row for this action and is relying on the persisted resume outcome.',
  ...
  return {
    ...
    durableStateDetail:
      'Xero could not find a durable approval row for this action inside the current selected-project snapshot.',
    durableUpdatedAt: null,
  }
}
...
function getCheckpointResumeState(card: CheckpointControlLoopCardAccumulator): {
  ...
  if (card.approval?.isPending) {
    return {
      ...
      resumeDetail: 'Xero is waiting for operator input before this action can resume the run.',
  ...
  return {
    ...
    resumeDetail: 'Xero has not recorded a durable resume outcome for this action yet.',
...
function getCheckpointControlLoopDetail(card: CheckpointControlLoopCardAccumulator): string {
  return (
    ...
    normalizeText(card.latestResume?.summary) ??
    normalizeText(card.evidence[0]?.summary) ??
    'Xero is tracking this checkpoint boundary from the selected-project snapshot and bounded runtime evidence window.'
  )
}
