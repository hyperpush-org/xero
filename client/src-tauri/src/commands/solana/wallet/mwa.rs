...
fn expo_app_json(ctx: &WalletScaffoldContext) -> String {
  ...
  "expo": {{
    ...
    "android": {{
      "package": "xyz.xero.{slug_underscore}"
    }},
    "ios": {{
      "bundleIdentifier": "xyz.xero.{slug_underscore}"
    }}
  }}
}}
"#,
...
export default function App() {{
  ...
  const onSign = useCallback(async () => {{
    try {{
      const sig = await signMessage('hello from Xero workbench')
      setSignature(sig)
    }} catch (err) {{
...
Mobile Wallet Adapter companion scaffold generated by Xero's Solana
workbench.
