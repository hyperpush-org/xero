...
const MIGRATION_022_GENERALIST_AGENT_LABEL_SQL: &str = r#"
    UPDATE agent_definitions
    SET display_name = CASE display_name WHEN 'Generalist' THEN 'Agent' ELSE display_name END,
        short_label = CASE short_label WHEN 'Generalist' THEN 'Agent' ELSE short_label END,
        updated_at = '2026-05-12T00:00:00Z'
    WHERE definition_id = 'generalist'
      AND (display_name = 'Generalist' OR short_label = 'Generalist');

    UPDATE agent_definition_versions
    SET snapshot_json = json_set(snapshot_json, '$.displayName', 'Agent', '$.shortLabel', 'Agent')
    WHERE definition_id = 'generalist'
      AND json_valid(snapshot_json)
      AND (
        json_extract(snapshot_json, '$.displayName') = 'Generalist'
        OR json_extract(snapshot_json, '$.shortLabel') = 'Generalist'
      );
"#;

fn migrate_generalist_agent_label(transaction: &Transaction<'_>) -> rusqlite_migration::HookResult {
    if !table_exists(transaction, "agent_definitions")?
        || !table_exists(transaction, "agent_definition_versions")?
    {
        return Ok(());
    }
    transaction.execute_batch(MIGRATION_022_GENERALIST_AGENT_LABEL_SQL)?;
    Ok(())
}
...
mod tests {
    ...
    fn builtin_agent_workflow_structure_backfills_missing_baseline_definitions() {
        ...
        assert_eq!(
            ...
            "hook should backfill missing Plan, Crawl, and Agent baseline rows"
        );
