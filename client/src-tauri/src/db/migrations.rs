...
mod tests {
    ...
    #[test]
    fn fresh_schema_seeds_only_executable_builtin_stage_policies() {
        let connection = migrate_to_latest_in_memory();
        let mut statement = connection
            .prepare(
                r#"
                SELECT adv.definition_id, adv.version, adv.snapshot_json
                FROM agent_definition_versions adv
                JOIN agent_definitions ad
                  ON ad.definition_id = adv.definition_id
                WHERE ad.scope = 'built_in'
                  AND json_type(adv.snapshot_json, '$.workflowStructure') = 'object'
                ORDER BY adv.definition_id, adv.version
                "#,
            )
            .expect("prepare built-in Stage policy query");
        let rows = statement
            .query_map([], |row| {
                Ok((
                    row.get::<_, String>(0)?,
            ...
            .expect("query built-in Stage policies");

        for row in rows {
            let (definition_id, version, snapshot_json) = row.expect("read built-in Stage policy");
            let snapshot: serde_json::Value = serde_json::from_str(&snapshot_json)
                .unwrap_or_else(|error| panic!("parse {definition_id}@{version}: {error}"));
            let policy = AutonomousAgentWorkflowPolicy::from_definition_snapshot(&snapshot)
                .unwrap_or_else(|| panic!("{definition_id}@{version} omitted workflowStructure"));
            policy.ensure_valid().unwrap_or_else(|error| {
                panic!(
                    "built-in Stage policy {definition_id}@{version} is not executable: {} ({})",
