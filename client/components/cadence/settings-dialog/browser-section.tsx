        ...
        {available.length === 0 ? (
          <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-border/70 bg-secondary/15 px-5 py-7 text-center">
            ...
            <p className="max-w-sm text-[12px] leading-[1.55] text-muted-foreground">
              Cadence didn't find any cookie sources on this machine. Install Chrome, Safari, Firefox, Edge, Brave, or Arc, then rescan.
            </p>
          </div>
        ) : (
    ...
    <section className="flex flex-col gap-3">
      ...
      <ul className="flex flex-col divide-y divide-border/50 overflow-hidden rounded-lg border border-border/60 bg-card/30">
        <DetailRow
          ...
          body="Cadence reads cookies from your installed browser's profile on disk. Nothing is uploaded — the import never leaves your device."
        />
        <DetailRow
          icon={Lock}
          title="Keychain prompt on first run"
          body="macOS may prompt once for Keychain access to decrypt cookies. Approve it to allow Cadence to read encrypted entries."
        />
        <DetailRow
          icon={RefreshCw}
...
function summarize(
  ...
  if (availableCount === 0) {
    return {
      tone: "muted",
      label: "No sources",
      body: "Cadence didn't detect any installed browsers. Install a supported browser and rescan to pull existing sessions in.",
    }
  }
  return {
    tone: "ok",
    ...
    } ready to import from. Pick a source below — Cadence reads cookies locally and never uploads them.`,
  }
}
