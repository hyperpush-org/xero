...
describe('ProviderCredentialsList', () => {
  ...
  it('renders a row per provider preset with the right action button when no credentials exist', () => {
    ...
    expect(within(xaiCard).getByRole('button', { name: /configure/i })).toBeInTheDocument()
    expect(within(xaiCard).queryByRole('button', { name: /sign in/i })).not.toBeInTheDocument()
