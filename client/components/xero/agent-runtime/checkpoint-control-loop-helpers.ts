...
export function getCheckpointControlLoopRecoveryAlertMeta(options: {
  ...
  if (options.notificationSyncPollingActive && options.trustSnapshot.syncState === 'degraded') {
    return {
      ...
      body: `Xero is still polling remote routes for blocked boundary ${displayValue(options.notificationSyncPollingBoundaryId, 'unknown')} and action ${displayValue(options.notificationSyncPollingActionId, 'unknown')} while preserving the last truthful sync summary. ${options.trustSnapshot.syncReason}`,
  ...
  if (options.notificationSyncPollingActive) {
    return {
      ...
      body: `Xero is polling remote routes for blocked boundary ${displayValue(options.notificationSyncPollingBoundaryId, 'unknown')} and action ${displayValue(options.notificationSyncPollingActionId, 'unknown')} while durable approval, broker, and resume truth remain visible here.`,
      variant: 'default' as const,
    }
  }
    ...
    return {
      ...
      detail:
        pendingOperatorIntent?.kind === 'resume'
          ? 'Resume request is in flight for this action. Xero will refresh durable state before updating this card.'
          : 'Decision persistence is in flight for this action. Xero keeps the last durable resume state visible until refresh completes.',
