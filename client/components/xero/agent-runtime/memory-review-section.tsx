...
export function MemoryReviewSection({
  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      ...
      setMessage({
        kind: 'error',
        text: getErrorMessage(error, 'Xero could not load reviewed memory.'),
      })
    }
  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      ...
      setMessage({
        kind: 'success',
        text: `Xero proposed ${response.createdCount.toLocaleString()} memory candidate${response.createdCount === 1 ? '' : 's'}.${skipped}${rejected}`.trim(),
      })
    } catch (error) {
      setMessage({
        kind: 'error',
        text: getErrorMessage(error, 'Xero could not extract memory candidates.'),
      })
    } finally {
  ...
  const handleUpdate = useCallback(
    async (
      ...
      try {
        ...
        setMessage({
          kind: 'error',
          text: getErrorMessage(error, 'Xero could not update memory.'),
        })
      } finally {
        setMutationStatus('idle')
      }
  ...
  const handleDelete = useCallback(
    async (memory: SessionMemoryRecordDto) => {
      ...
      try {
        ...
        setMessage({
          kind: 'error',
          text: getErrorMessage(error, 'Xero could not delete memory.'),
        })
      } finally {
        setMutationStatus('idle')
      }
