...
export type WebComposerContextIndicatorStatus =
	| "idle"
	| "loading"
	| "ready"
	| "stale"
...
export type WebComposerContextPressure =
	| "unknown"
	| "low"
	| "medium"
	| "high"
...
export interface WebComposerContextBudget {
	...
	pressure?: WebComposerContextPressure | null;
...
export interface WebComposerContextSnapshot {
	modelId?: string | null;
	budget?: WebComposerContextBudget | null;
	contributors?: readonly {
		kind?: string | null;
	}[];
}

export interface WebComposerContextIndicatorError {
	message?: string | null;
}

export interface WebComposerContextIndicatorProps {
	status: WebComposerContextIndicatorStatus;
	snapshot: WebComposerContextSnapshot | null;
	hasUserMessage?: boolean;
	error?: WebComposerContextIndicatorError | null;
}
...
function getBudgetLabel(
	...
	if (budget.pressure === "over") {
		const overflow = Math.max(
			0,
			(budget.estimatedTokens ?? 0) - (budget.effectiveInputBudgetTokens ?? 0),
		);
...
function getContextPressureStroke(pressurePercent: number): string {
	...
	const normalizedPressure =
		((Math.min(
			100,
			Math.max(CONTEXT_DEPLETION_COLOR_START_PERCENT, pressurePercent),
		) -
			CONTEXT_DEPLETION_COLOR_START_PERCENT) /
			(100 - CONTEXT_DEPLETION_COLOR_START_PERCENT)) *
		100;
	const depletedWeight = Math.max(
		0,
		Math.min(100, Math.round(normalizedPressure)),
	);
...
function getErrorMessage(
	error: WebComposerContextIndicatorProps["error"],
): string | null {
	const message = error?.message?.trim();
	return message && message.length > 0 ? message : null;
}

export function WebComposerContextIndicator({
	status,
	snapshot,
	hasUserMessage = true,
	error = null,
	...
	const knownBudget = Boolean(
		budget?.knownProviderBudget && budget.pressurePercent != null,
	);
	const hideBaselineUsage = knownBudget && !hasUserMessage;
	const pressure =
		knownBudget && !hideBaselineUsage
			? Math.min(100, budget?.pressurePercent ?? 0)
			: 0;
	...
	const shouldAnimateRefresh =
		(status === "loading" || status === "stale") && !knownBudget;
	const ringStyle =
		knownBudget && status !== "error" && budget?.pressure !== "over"
			? { stroke: getContextPressureStroke(pressure) }
			: undefined;
	const errorMessage =
		status === "error" && !snapshot ? getErrorMessage(error) : null;
	...
	const ariaValueText = knownBudget
		? `${remainingPercent} percent context remaining for ${
				snapshot?.modelId ?? "the selected model"
			}`
		: errorMessage
			? `${label}: ${errorMessage}`
			: label;
		...
		<Tooltip>
			<TooltipTrigger asChild>
				<button
					type="button"
					className={cn(
						"inline-flex h-7 min-w-7 items-center justify-center rounded-md px-1.5 text-[12px] font-medium text-muted-foreground transition-colors",
