...
export interface MarkdownTheme {
  /** Shiki theme name used for code-block tokenization (e.g. 'github-dark'). */
  shiki: string
  /** Active appearance, used to pick Mermaid's dark vs. default theme. */
  appearance: 'dark' | 'light'
}

const DEFAULT_MARKDOWN_THEME: MarkdownTheme = { shiki: 'github-dark', appearance: 'dark' }
...
export interface MarkdownProps {
  ...
  /** Theme used for code highlighting and Mermaid rendering. */
  theme?: MarkdownTheme
}
    ...
    >
      {segments.map((segment, index) =>
        segment.kind === 'code' && segment.lang === 'mermaid' ? (
          <MermaidBlock key={index} code={segment.code} streaming={streaming} scale={resolvedScale} theme={theme} />
        ) : segment.kind === 'code' ? (
          <CodeBlock key={index} code={segment.code} lang={segment.lang} streaming={streaming} scale={resolvedScale} theme={theme} />
        ) : (
          <Fragment key={index}>
...
function CodeBlock({ code, lang, streaming = false, scale, theme }: CodeBlockProps) {
...
function MermaidBlock({ code, streaming, scale, theme }: MermaidBlockProps) {
