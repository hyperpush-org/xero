defmodule Joe.GitHubAuth do
  @moduledoc """
  Server-owned GitHub OAuth flow for Cadence desktop account linking.
    ...
    cond do
      error_code = params["error"] ->
        error =
          github_error(
            error_code,
            params["error_description"] || "GitHub did not authorize this Cadence login."
          )
  ...
  defp load_dotenv_files do
    if System.get_env("CADENCE_GITHUB_AUTH_SKIP_DOTENV") in ["1", "true"] do
      :ok
    else
  ...
  defp missing_env(name) do
    {:error,
     error(
       ...
       "#{name} is not set on the Cadence server. Configure server/.env and restart Phoenix."
     )}
  end
  ...
  defp server_public_url do
    case public_url_env("CADENCE_SERVER_PUBLIC_URL") || public_url_env("PUBLIC_SERVER_URL") do
      value when is_binary(value) ->
