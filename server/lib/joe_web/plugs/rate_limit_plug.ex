defmodule JoeWeb.RateLimitPlug do
  @moduledoc """
  Per-IP rate limit plug. Bucket size + window come from app config under
  `:joe, Joe.RateLimiter` (see `config/runtime.exs`).

  Override per-route by passing `:scale_ms` and `:limit` in the plug opts:

      plug JoeWeb.RateLimitPlug, scale_ms: :timer.minutes(1), limit: 10
  """
  ...
  def call(conn, opts) do
    ...
    limit =
      Keyword.get_lazy(opts, :limit, fn ->
        Application.get_env(:joe, Joe.RateLimiter, [])
        |> Keyword.get(:per_minute, 60)
      end)

    key = "ip:" <> client_ip(conn)

    case Joe.RateLimiter.hit(key, scale, limit) do
      {:allow, _count} ->
        conn
